...
const CustomTable = ({ columns, datas, rowClick, pagination }) => {
    ...
    return <table>
        ...
        <tbody>
            {
                datas.slice(currentPage * 10, (currentPage * 10) + 10).map((d, ind) => <tr key={ind} onClick={(e) => {
        ...
        <tfoot>
            <tr className="custom-table-footer">
                {pagination && pages.length > 0 && <td className="custom-pagination-container" style={{ width: pages.length > 5 ? 300 : 100 + (pages.length * 40) }}>
