...
import { AdminsColumns, ApplicationsColumns, BillingColumns } from '../../../Constants/TableColumns';
...
const AdminsDetail = ({ data }) => {
    ...
    return data ? <>
        <section>
            <CustomButton onClick={openConfirm}>
                데이터 삭제
            </CustomButton>
        </section>
        <section>
        ...
        <section>
            <h2>Applications</h2>
            <CustomTable columns={ApplicationsColumns} datas={testData2} pagination numPerPage={5}/>
        </section>
        <section>
            <h2>Sub Admins</h2>
            <CustomTable columns={AdminsColumns} datas={testData} pagination numPerPage={5}/>
        </section>
        <CustomConfirm visible={confirmVisible} footer={true} cancelCallback={closeConfirm} confirmCallback={confirmCallback}>
            정말 삭제하시겠습니까?
        </CustomConfirm>
    </> : <Redirect to="/Admins" />
}
