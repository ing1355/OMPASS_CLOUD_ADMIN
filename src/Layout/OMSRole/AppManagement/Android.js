...
import { CustomAxiosGet, CustomAxiosPost, CustomAxiosPut } from '../../../Functions/CustomAxios';
...
const Android = () => {
    ...
    useLayoutEffect(() => {
        CustomAxiosGet(getAppManagementApi, data => {
            console.log(data)
    ...
    const uploadApk = (e) => {
        ...
        if (tableData.find(t => t.version === appVersion)) {
            CustomAxiosPut(updateAppManagementApi, formData, res => {
                console.log(res);
                setTableLoading(false);
            }, err => {
                console.log(err)
